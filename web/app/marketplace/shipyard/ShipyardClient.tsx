    ...
    >
      ...
      >
        ...
        <div className="px-5 pt-4 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          {isNext && (
            <button onClick={onBuy} disabled={!canAfford || isPending} className="btn-ghost w-full disabled:opacity-30 mt-3">
              {isPending ? 'Upgrading…' : canAfford ? `Upgrade · ${ship.cost.toLocaleString()} ⟡` : `${(ship.cost - doubloons).toLocaleString()} ⟡ short`}
            </button>
          )}
        </div>

        <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-5" style={{ paddingBottom: 'calc(5rem + env(safe-area-inset-bottom, 0px))' }}>
