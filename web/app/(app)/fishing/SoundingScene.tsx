// THE SOUNDING — the /fishing waiting scene, drawn once and used twice.
//
// Rendered by loading.tsx (the route's Suspense fallback, a SERVER component)
// and by FishingAssetGate (a CLIENT overlay that holds past that boundary until
// the tile art has actually decoded). They have to be pixel-identical or the
// handoff between them flickers, so neither draws it and both call this.
//
// No hooks and no 'use client' of its own on purpose: that lets a server
// component and a client component both import it.
//
// IT IS THE PAGE'S OWN BACKDROP. The fixed layer here is copied stop for stop
...
// fade in over a scene that was already there. Keep the two in step: if
// FishingHub's backdrop changes, change it here.
//
// Everything that moves is a CSS keyframe (see "THE SOUNDING" in globals.css)
...
export default function SoundingScene() {
  return (
    <>
