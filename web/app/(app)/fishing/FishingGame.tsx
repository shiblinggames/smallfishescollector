...
const AncientSlainCinematic = dynamic(() => import('./AncientSlainCinematic'), { ssr: false })
...
import FishCollectionDrawer from './FishCollectionDrawer'
...
// RARITY, fishImageUrl, FishImg, TrophyMark and the result card itself now live
// in components/CatchResultCard so the sea map can render the SAME card rather
// than a second, lesser one. Moved verbatim; see that file for the why.
import { RARITY, fishImageUrl, FishImg, TrophyMark, ResultCard } from '@/components/CatchResultCard'
...
// XPBarDisplay moved to components/FishingXPBar so the sea map's cast overlay
// can wear the same bar. Imported above.
...
export default function FishingGame({
  ...
  return (
    <div className="fixed left-0 right-0 top-[44px] bottom-[60px] sm:top-[60px] sm:bottom-0" style={{ background: '#08121c', zIndex: 40, display: 'flex', justifyContent: 'center' }}>
      ...
      {/* ── Collection drawer ── */}
      <AnimatePresence>
        {/* THE COLLECTION. Lives in its own file now, because the ocean hub
            shows the same log and zone completion pays a reward — one payout,
            one implementation. The drawer owns its drag-to-dismiss; everything
            else is still this component's state. */}
        {collectionOpen && (
          <FishCollectionDrawer
            ...
            setTappedFishId={setTappedFishId}
            uncheckedNewFishIds={uncheckedNewFishIds}
            setUncheckedNewFishIds={setUncheckedNewFishIds}
            confirmPrestigeZone={confirmPrestigeZone}
            setConfirmPrestigeZone={setConfirmPrestigeZone}
            handleClaimZoneReward={handleClaimZoneReward}
            handlePrestige={handlePrestige}
            setReleasingAncient={setReleasingAncient}
