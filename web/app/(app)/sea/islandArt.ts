...
export const PALETTES: IslePalette[] = [
...
export function paletteOf(id: string): IslePalette {
  const h = Math.imul(seedOf(id) ^ 0x5bf03635, 2246822519) >>> 0
  return PALETTES[h % PALETTES.length]
}

/**
 * ── THE DIAL, AND THE CLAMP THAT HAS TO COME LAST ───────────────────────────
 *
 * Warm and pale one way, cool and dark the other. At module scope so the check
 * script can walk it; see scripts/check-palettes.
 *
 * ── THIS FUNCTION TOOK THE WHOLE CHART DOWN. READ THIS BEFORE EDITING IT ────
 *
 * It clamped each channel to 0..255 and THEN multiplied by `dim`. But `dim` is
 * `1 - k * 0.055` and k runs down to -1, so on any island whose `chr` is under
 * a half the last step BRIGHTENS — and a channel already clamped to 255 came
 * back out at 260.
 *
 * `260 << 16` is 0x1040000, which prints as SEVEN hex digits. `#104f1ca` is not
 * a colour, and `addColorStop` throws SyntaxError on it.
 *
 * What that looked like from the outside is worth more than the arithmetic. The
 * throw was inside `bakeIsland`, so it was inside `place()`, so it was inside
 * the loop that places every island — and everything built after that loop,
 * including the GPU handle itself, was never built. SeaMap reaches the handle
 * through optional chaining, so every camera() and skipper() call after it
 * silently did nothing: the chart froze with no boat on it, while the water
 * carried on animating off the Pixi ticker's own clock. A dead colour presents
 * as a dead camera, three files away.
 *
 * It also needed the Crew Hall's palette AND the Crew Hall's seed together, so
 * nine islands baked and the tenth did not.
 *
 * THE CLAMP GOES LAST, after every multiply, and the bytes are masked on the
 * way into the string as well. Belt and braces on purpose: this is a hex string
 * handed to a parser that throws, from a function every colour on the land runs
 * through.
 */
export function toneHex(hex: string, warmCool: number): string {
  ...
  return `#${(
    (byte(r * (1 - k * 0.085) * dim) << 16)
    | (byte(g * (1 - k * 0.025) * dim) << 8)
    | byte(b * (1 + k * 0.11) * dim)
...
/** Where an island sits inside its palette's family, 0..1. Exported for the
 *  check — it has to drive the dial with the same number the bake does, or it
 *  is testing a function nobody calls. */
export function paletteChr(id: string): number {
  return ((seedOf(id) >>> 9) % 1000) / 1000
}
