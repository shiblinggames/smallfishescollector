...
async function ensureDailyContent(zone: ZoneKey, date: string): Promise<DailyExpeditionRow> {
  ...
  if (error || !inserted) throw new Error('Failed to generate expedition content')
...
export async function resolveFinalLoot(
  ...
  const loot: LootResult = {
    ...
    base: rollResult.base,
    crewBonus: rollResult.crewBonus,
    effectiveStat: rollResult.effectiveStat,
    finalScore,
    successBonus,
    cardVariantId,
    cardName,
