...
function StatCell({ label, value }: { label: string; value: number }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5cqw' }}>
      ...
      <span className="font-pirata" style={{ fontSize: '12cqw', color: '#3d2b1a', lineHeight: 1 }}>
        {value}
      </span>
    </div>
  )
    ...
    frame = (
      <div className="relative overflow-hidden" style={{ width: W, height: H, borderRadius: R, background: artBg[artEffect] ?? '#c8a870', containerType: 'inline-size' }}>
        ...
        <div className="absolute left-0 right-0 bottom-0 pointer-events-none overflow-hidden" style={{ height: '40%', zIndex: 6, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '0 8% 4%', gap: '1cqw' }}>
          <p className="font-pirata text-center w-full" style={{ fontSize: nameFontSize, color: '#3d2b1a', letterSpacing: '0.04em', lineHeight: 1.1 }}>
            {name}
          </p>
          {rarity && (
            <p className="font-karla font-700 text-center uppercase tracking-[0.12em]" style={{ fontSize: '6.5cqw', color: rarityColor!, textShadow: '0 1px 0 rgba(0,0,0,0.9), 0 0 6px rgba(0,0,0,0.7), 0 1px 4px rgba(0,0,0,0.8)' }}>
              {rarity}
            </p>
          )}
          {stats && (
