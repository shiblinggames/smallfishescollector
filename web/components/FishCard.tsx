  ...
  if (unowned) {
    ...
    frame = (
      <div className="relative overflow-hidden" style={{ width: W, height: H, borderRadius: R, background: artBg[artEffect] ?? '#c8a870' }}>
        ...
        <div className="absolute left-0 right-0 pointer-events-none" style={{ zIndex: 6, bottom: 0, padding: '4px 10px 16px' }}>
          <p className="font-cinzel font-700 text-center" style={{ fontSize: '1.25rem', color: '#1a1008', marginBottom: 2, letterSpacing: '0.04em' }}>
            {name}
          </p>
          {rarity && (
            <p className="font-karla font-700 text-center uppercase tracking-[0.12em]" style={{ fontSize: '0.72rem', color: rarityColor!, marginBottom: stats ? 5 : 0 }}>
              {rarity}
            </p>
          )}
          {stats && (
