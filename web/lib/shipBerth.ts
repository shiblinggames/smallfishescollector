// The Sixth Berth — a purchasable Man-o-War crew slot (5 → 6), unlocked by
// beating Raid 7 (the Blockade). A deliberate power-spike price: it opens the
// full six-crew bench Don Finleone's six phases demand.
export const SIXTH_BERTH_COST = 1_000_000

// The Expanded Armory — a purchasable extra raid-item mount (Don Finleone's
// shipwright), unlocked by beating Raid 8 (the Throne). Same power-spike price
// as the berth: one more piece of gear working every fight, bought once.
export const ARMORY_EXPANSION_COST = 1_000_000
