...
export const GAUNTLET_DEPTH_UNLOCKS: GauntletDepthUnlock[] = [
  { depth: 5,  name: 'Auto Catcher',        blurb: 'A permanent upgrade for your Auto Caster: it reels in common & uncommon fish on its own.', where: 'Buy the upgrade in the Locker shop.' },
...
export const GAUNTLET_BOONS: GauntletBoon[] = [
  ...
  { id: 'krakens_grip', image: '/gauntlet/boons/krakens_grip.png', name: "Kraken's Grip", gauntlet: 'don', flavor: 'A landed shot calls the deep up around their hull — and the deep does not let go.', rarity: 'legendary', tiers: [
    { desc: 'Each hit coils tighter and can seize; the 5th always does. Held a turn, crushed 2% of its max HP per coil', detail: 'Every hit you land coils the deep tighter around the hull, and every coil raises the chance it closes — slim at the first, certain at the fifth. When it closes, the ship loses its next turn and is crushed for 2% of its OWN maximum HP per coil that had built up, so a late close hits far harder than an early one. Because the odds and the crush both ride the same count, luck decides the rhythm and never the damage. The coils are around that hull, so a fresh enemy starts the count again. Bosses are not immune, and since the crush is a share of their own hull, the biggest ships take the most.', effect: { kind: 'gripStacks', hits: 5, turns: 1, crushPerStack: 0.02 } },
    { desc: 'Coils close by the 4th hit, hold 2 turns, and crush 3% of its max HP per coil', detail: 'The deep closes sooner and holds longer: the count runs to four instead of five, the hull loses its next TWO turns, and the crush rises to 3% of its own maximum HP per coil that had built up. Every coil still raises the chance it closes early, and the crush still pays for the coils spent, so the roll only ever moves the rhythm.', effect: { kind: 'gripStacks', hits: 4, turns: 2, crushPerStack: 0.03 } },
  ] },
...
export const CONFLUENCES: Confluence[] = [
  ...
  },
  {
    ...
    levels: [
      { desc: 'The coils close a hit sooner; +14% damage', effects: [{ kind: 'gripStacks', hits: 4, turns: 1, crushPerStack: 0.02 }, { kind: 'damageMult', mult: 1.14 }] },
      { desc: 'A hit sooner again, held 2 turns; +22% damage', effects: [{ kind: 'gripStacks', hits: 3, turns: 2, crushPerStack: 0.03 }, { kind: 'damageMult', mult: 1.22 }] },
    ],
  },
  {
