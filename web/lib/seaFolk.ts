...
/**
 * Points to reach each tier.
 *
 * Thirty points to the top, which is a month of daily chats or rather less
 * with gifts. Long on purpose: nine regulars at that pace is a tail measured
 * in seasons of play, and there is nothing to rush toward because nothing
 * expires.
 */
export const TIER_AT: Record<FolkTier, number> = { 0: 0, 1: 4, 2: 10, 3: 18, 4: 30 }
...
export type Folk = {
   ...
   *
   * Three per tier, chosen by what has not been heard yet, so a run of daily
   * visits finds something new before it starts going round again. A stranger
   * gets the guarded version; by the top tier they are telling you things they
   * would not tell the harbour.
   */
