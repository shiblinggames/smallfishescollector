...
- **Fishing XP comes from catches, trawls, and Finn's jobs, in that order of weight.**
  ...
  amount each (`xp` in `lib/finnQuests.ts`), in a tight band per chapter (80 to 110 in the
...
- **A perfect's XP bonus is SAID, not just paid (2026-09-17).** `catchXP()` is 1.2x on a
  ...
  Reported, never banked. The floating number off the boat is the TOTAL, gold and marked
  PERFECT. **The result card's tally is ADDITIVE:** `reelIn` returns `xpCatch`,
  `perfectBonusXP` and `xpStreak`, all after the multipliers, and they sum exactly to
  `xpGained`; the card shows those three (`+N XP`, `+N Perfect`, `+N Streak`), so a reader
  adding them gets the number that flew off the boat. The first cut put the total in the
